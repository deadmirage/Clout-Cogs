...
class Points:
        ...
        if not name:
            ...
            for x in names:
                ...
                if x is None:
                    ...
                    self.db[server.id][x.id] = OrderedDict(
                        {"Name": x.display_name, "Clout": 0, "Lifetime Gain": 0, "Lifetime Loss": 0})
                    self.save_db()
            ...
            if name.id in self.db[server.id]:
                ...
                self.db[server.id][name.id] = OrderedDict(
                    {"Name": name.display_name, "Clout": 0, "Lifetime Gain": 0, "Lifetime Loss": 0})
                self.save_db()
        ...
        if not name:
            ...
            for x in names:
                ...
                if x is None:
                    ...
                    self.db[server.id][x.id]["Clout"] += points
            ...
            self.db[server.id][name.id]["Clout"] += points
    ...
    async def _list(self, ctx):
        ...
        try:
            ...
            rows = sorted([[db[x]["Name"], db[x]["Lifetime Gain"], db[x]["Lifetime Loss"], db[x]["Clout"]]
    ...
    async def balance(self, ctx, name: discord.Member=None):
        """Allows to show the clout of a user. Defaults to author."""
        server = ctx.message.server
        author = ctx.message.author
        if name is None:
            name = author
        ...
        if name.id not in self.db[server.id]:
            ...
            balance = self.db[server.id][name.id]["Clout"]
            await self.bot.say("{} has a current clout of {} points. "
